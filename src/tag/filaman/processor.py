...
class FilamanTagProcessor(NdefTagProcessor):
    ...
    def process_ndef(self, scan_result: ScanResult, ndef_records: list[NdefRecord]) -> GenericFilament | None:
        for record in ndef_records:
            if record.mime_type == "application/json":
                parse = self.__filaman_parse_payload(record.payload)

                if parse is not None:
                    return parse
        
        self.logger.error("Filaman processing failed: No valid Filaman NDEF record found")
        return None

    def __filaman_parse_payload(self, payload : bytes) -> GenericFilament | None:
        if None == payload or not isinstance(payload, (bytes, bytearray)):
            self.logger.error("Filaman payload parsing failed: Invalid payload parameter")
            return None

        try:
            payload_str = payload.decode('utf-8')
            self.logger.debug(f"Filaman JSON payload: {payload_str}")

            data = json.loads(payload_str)

            if not isinstance(data, dict):
                self.logger.error(f"Filaman payload parsing failed: JSON data is not a dict, got {type(data)}")
                return None

            if data.get('protocol') != 'filaman':
                self.logger.error(f"Filaman payload parsing failed: Invalid protocol '{data.get('protocol')}', expected 'filaman'")
                return None
            ...
            if max_temp < min_temp:
                self.logger.error("Filaman payload parsing failed: Invalid temperature values")
                return None
            ...
            return GenericFilament(
                ...
                unique_id=GenericFilament.generate_unique_id("Filaman", brand, main_type, subtype, color_argb),
                manufacturer=brand,
                type=main_type,
            ...
            self.logger.exception("Filaman payload parsing failed: Invalid JSON: %s", str(e))
            return None
        except Exception as e:
            self.logger.exception("Filaman payload parsing failed: %s", str(e))
            return None
